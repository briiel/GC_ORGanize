// Browser-native AES-256-GCM encryption mapping to the backend transport standard using environment-configured pre-shared key
...
export class PayloadEncryptionService {
  ...
  private importKey(): Promise<CryptoKey> {
    ...
    if (!hex || hex.length !== 64) {
      return Promise.reject(
        new Error(
          '[PayloadEncryption] payloadEncryptionKey must be 64 hex chars.',
        ),
      );
    }
  ...
  async encrypt(data: any): Promise<string> {
    ...
    const ciphertextBytes = new Uint8Array(
      ciphertextWithTag,
      0,
      ciphertextWithTag.byteLength - 16,
    );
  ...
  async decrypt(encryptedStr: string): Promise<any> {
    ...
    if (parts.length !== 3)
      throw new Error('[PayloadEncryption] Invalid encrypted payload format.');
    ...
    const plaintextBuf = await crypto.subtle.decrypt(
      { name: this.algorithm, iv: iv as any, tagLength: this.tagLength },
      key,
      combined as any,
    );
